...
function emitSource(
  ...
  const lines: string[] = [];
  lines.push('// Auto-generated by json-schema-zod-tools.');
  ...
  for (const schemaName of sortKeys(model.components.schemas)) {
    ...
    lines.push(`const ${schemaVarName} = ${schemaExpr};`);
  }
  ...
  for (const pathKey of sortKeys(model.paths)) {
    ...
    if (!methods) continue;
    lines.push(`    ${toPropertyKey(pathKey)}: {`);
    for (const method of sortKeys(methods)) {
      ...
      if (pathParamsOverride !== undefined) {
        lines.push(`        path: ${pathParamsOverride},`);
      } else {
        lines.push('        path: z.object({');
        for (const paramName of sortKeys(op.path)) {
          ...
          lines.push(`          ${toPropertyKey(paramName)}: ${expr},`);
        }
        lines.push('        }),');
      }
      ...
      if (queryParamsOverride !== undefined) {
        lines.push(`        query: ${queryParamsOverride},`);
      } else {
        lines.push('        query: z.object({');
      ...
      if (op.requestBody) {
        ...
        if (requestBodyOverride !== undefined) {
          lines.push(`        requestBody: ${requestBodyOverride},`);
        } else {
          ...
          for (const mediaType of sortKeys(op.requestBody)) {
            ...
            lines.push(`          ${toPropertyKey(mediaType)}: ${expr},`);
          }
          lines.push('        },');
        }
      }
      ...
      if (responsesOverride !== undefined) {
        lines.push(`        responses: ${responsesOverride},`);
      } else {
        ...
        for (const statusCode of sortKeys(op.responses)) {
          ...
          lines.push(`          ${toPropertyKey(statusCode)}: ${expr},`);
        }
        lines.push('        },');
      }
  ...
  lines.push('    schemas: {');
  for (const schemaName of sortKeys(model.components.schemas)) {
    lines.push(`      ${toPropertyKey(schemaName)}: ${componentSchemaVarNames[schemaName]},`);
  }
  lines.push('    },');
  lines.push('  },');
